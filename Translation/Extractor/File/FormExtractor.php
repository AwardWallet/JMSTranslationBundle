...
class FormExtractor implements FileVisitorInterface, \PHPParser_NodeVisitor
{
        ...
        if ($node instanceof \PHPParser_Node_Expr_MethodCall) {
         ...
         if ($node instanceof \PHPParser_Node_Expr_Array) {
            ...
            foreach ($node->items as $item) {
                ...
                if ('label' !== $item->key->value && 'help' !== $item->key->value && 'empty_value' !== $item->key->value && 'choices' !== $item->key->value && 'invalid_message' !== $item->key->value) {
                    continue;
                }
    ...
    private function parseItem($item, $domain = null)
    {
        ...
        $docComment = !empty($docComment) ? $docComment : $item->value->getDocComment();
        if ($docComment) {
